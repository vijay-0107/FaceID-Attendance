...
def create_dataset(username):
	...
	while(True):
		...
		for face in faces:
			...
			cv2.imwrite(directory+'/'+str(datetime.datetime.now())+'.jpg'	, face_aligned)
...
def predict(face_aligned,svc,threshold=0.3):
...
def mark_your_attendance(request):
	...
	while(True):
		...
		#Showing the image in another window
		#Creates a window with window name "Face" and with the image img
		cv2.imshow("Mark Attendance - In",frame)
...
def mark_your_attendance_out(request):
	...
	while(True):
		...
		#Showing the image in another window
		#Creates a window with window name "Face" and with the image img
		cv2.imshow("Mark Attendance- Out",frame)
